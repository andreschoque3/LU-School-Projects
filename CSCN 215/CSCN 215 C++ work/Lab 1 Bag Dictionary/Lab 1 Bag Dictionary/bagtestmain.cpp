...
int main(int argc, char** argv) {
	...
	ABag<KVpair<string, int>> myBag2; // Second test but reversing string and int. 
	KVpair<int, string> test1(700,"testing"); // Using "testing" and '700' as test subjects for the bag dictionary.
	KVpair<string, int> test2("Second", 950); // Using second test for the bag dictionary.
	KVpair<int, string> temp1; // calling into the function temp 1 and temp 2.
	KVpair<string, int> temp2; 

	myBag += test1; // myBag passes test1 in in order to perform the test.
	myBag.inspectTop(temp1); // Using function "inspect top" and running the temporary function.
		cout << "The new value on top of the bag is: " << temp1.value() << endl; // Comments in order to show the test.

	myBag2 += test2; // using the same strategy as line 136.
	myBag2.inspectTop(temp2); // Again using function "inspect top" and running the 2nd temporary function.
		cout << "The new value on top of the bag is: " << temp2.value() << endl; // Comments that demonstrate we are testing.
